...
contract('ERC721BasicToken', function (accounts) {
  ...
  describe('like a ERC721BasicToken', function () {
    ...
    beforeEach(async function () {
      ...
      let transactionData = abi.simpleEncode(
      ...
      transactionData = abi.simpleEncode(
        'mint(address,uint256)',
        creator,
        secondTokenId
      );
    ...
    describe('balanceOf', function () {
      describe('when the given address owns some tokens', function () {
        it('returns the amount of tokens owned by the given address', async function () {
          const transactionData = abi.simpleEncode(
            'balanceOf(address)',
            creator,
          );

          let balance = await this.token.sendTransaction({
            data: transactionData.toString('hex'),
            from: creator,
          });
          ...
          balance = await this.originalToken.sendTransaction({
            data: transactionData.toString('hex'),
            from: creator,
          });
      ...
      describe('when the given address does not own any tokens', function () {
        it('returns 0', async function () {
          ...
          const balance = await this.token.sendTransaction({
            data: transactionData.toString('hex'),
            from: creator,
          });
